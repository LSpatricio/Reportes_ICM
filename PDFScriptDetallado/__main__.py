...
PDFS_FINIQUITOS_DETALLADOS_DIR = migrate_legacy_directory(
    PDFS_FINIQUITOS_DIR,
    "PDF Finiquitos Normales",
    "PDF Finiquitos Detallados"
)
...
SCRIPT_NAME="PDFScriptDetallado"

apiConfigFile    = os.path.join(configDir, "api.json")                    #ICM API configuration
scriptConfigFile = os.path.join(configDir, "ConfigPDFScriptDetallado.json")  #script specific config
...
def draw_text(canvas_obj, x, y, text, font_name="Helvetica", font_size=9, max_width=None, min_size=6, trim_overflow=False):
    text = "" if text is None else str(text)
    if max_width:
...
def draw_center_text(canvas_obj, center_x, y, text, font_name="Helvetica", font_size=9, max_width=None, min_size=6):
    text = "" if text is None else str(text)
    if max_width:
        font_size = fit_font_size(text, font_name, max_width, font_size, min_size=min_size)
    canvas_obj.setFont(font_name, font_size)
...
def calculate_flujo_mensual(row):
    return (
        ...
        + safe_float(get_row_value(row, "aportcajaaahorro_mensual"), 0.0)
...
def build_second_signature_text(row):
    ...
    return (
        "Recibí de Cadena Comercial OXXO SA de CV, la cantidad de "
        f"{total_comision} por concepto de Pago Total de Comisión Mercantil "
        f"correspondiente al Mes de {mes} del Año {anio} acreditando con la "
        ...
        "Facturación correspondiente a este mes."
    )
...
def resolve_input_csv_path(config_section):
    configured_path = get_config_value(config_section, "LOCAL_TEMPLATE_CSV_PATH", "")
    if configured_path:
        normalized = os.path.expandvars(os.path.expanduser(str(configured_path).strip()))
        relative_like_path = normalized.lstrip("/\\")
        candidates = []

        if normalized:
            candidates.extend([
                resolve_path(normalized, runtimeProjectRoot),
                resolve_path(normalized, projectRoot),
            ])

        if relative_like_path and relative_like_path != normalized:
            candidates.extend([
                os.path.normpath(os.path.join(runtimeProjectRoot, relative_like_path)),
                os.path.normpath(os.path.join(projectRoot, relative_like_path)),
            ])

        data_relative = re.sub(r"^(data[/\\\\])", "", relative_like_path, flags=re.IGNORECASE)
        if data_relative != relative_like_path:
            candidates.extend([
                os.path.normpath(os.path.join(DATA_DIR, data_relative)),
                os.path.normpath(os.path.join(dataDir, data_relative)),
            ])
    else:
        candidates = [
            os.path.join(dataDir, "CSVparaPDFDetallado"),
            os.path.join(dataDir, "CSVparaPDFDetallado.csv"),
            os.path.join(DATA_DIR, "CSVparaPDFDetallado"),
            os.path.join(DATA_DIR, "CSVparaPDFDetallado.csv"),
        ]
    ...
    return deduped_candidates[-1] if deduped_candidates else os.path.join(DATA_DIR, "CSVparaPDFDetallado.csv")
...
def find_default_template_path():
    ...
    preferred_pdf_names = [
        "template nuevo finiquito detallado sin datos.pdf",
        "template nuevo finiquito detallado con datos.pdf",
        "template pdf finiquito detallado sin datos.pdf",
        "pdf finiquito detallado con datos.pdf",
    ]
    lowered_to_original = {file_name.lower(): file_name for file_name in pdf_files}
    for preferred_name in preferred_pdf_names:
        matched = lowered_to_original.get(preferred_name)
        if matched:
            return os.path.join(templateDir, matched)

    for preferred_name in pdf_files:
        lowered = preferred_name.lower()
        if "detallado" in lowered and ("sin datos" in lowered or "sindatos" in lowered or "blank" in lowered):
            return os.path.join(templateDir, preferred_name)

    for preferred_name in pdf_files:
        lowered = preferred_name.lower()
        if "detallado" in lowered and ("con datos" in lowered or "condatos" in lowered):
            return os.path.join(templateDir, preferred_name)

    return os.path.join(templateDir, pdf_files[0])


def resolve_local_template_path(config_section):
    configured_template = get_config_value(config_section, "LOCAL_TEMPLATE_PATH", "")
    if configured_template:
        normalized = os.path.expandvars(os.path.expanduser(str(configured_template).strip()))
        candidate_paths = [
            resolve_path(normalized, runtimeProjectRoot),
            resolve_path(normalized, projectRoot),
            normalized if os.path.isabs(normalized) else None,
        ]
        for candidate in candidate_paths:
            if candidate and os.path.exists(candidate):
                return candidate

    default_template = find_default_template_path()
    if default_template and os.path.exists(default_template):
        return default_template

    return resolve_path(configured_template, runtimeProjectRoot) if configured_template else ""
...
def find_default_rdl_path():
    ...
    preferred_detallado_rdl = os.path.join(templateDir, "Finiquito_Detallado.rdl")
    if os.path.exists(preferred_detallado_rdl):
        return preferred_detallado_rdl
    ...
    lowered_to_original = {file_name.lower(): file_name for file_name in rdl_files}
    preferred_rdl = lowered_to_original.get("finiquito.rdl")
    if preferred_rdl:
        return os.path.join(templateDir, preferred_rdl)
...
def get_xml_local_name(element):
    return str(element.tag).split("}", 1)[-1]


def get_child_mm_points(element, child_name, namespace):
    child = element.find(f"rdl:{child_name}", namespace)
    if child is None or not child.text:
        return None
    return mm_to_points(child.text)


def collect_tablix_textboxes(tablix_node, namespace, base_left_pt, base_top_pt, collector):
    tablix_body = tablix_node.find("rdl:TablixBody", namespace)
    if tablix_body is None:
        return

    column_nodes = tablix_body.findall("./rdl:TablixColumns/rdl:TablixColumn", namespace)
    row_nodes = tablix_body.findall("./rdl:TablixRows/rdl:TablixRow", namespace)

    column_offsets = []
    current_left = 0.0
    for column in column_nodes:
        column_offsets.append(current_left)
        current_left += get_child_mm_points(column, "Width", namespace) or 0.0

    row_offsets = []
    current_top = 0.0
    for row in row_nodes:
        row_offsets.append(current_top)
        current_top += get_child_mm_points(row, "Height", namespace) or 0.0

    for row_index, row in enumerate(row_nodes):
        cell_nodes = row.findall("./rdl:TablixCells/rdl:TablixCell", namespace)
        for col_index, cell in enumerate(cell_nodes):
            cell_contents = cell.find("rdl:CellContents", namespace)
            if cell_contents is None:
                continue

            cell_base_left = base_left_pt + (column_offsets[col_index] if col_index < len(column_offsets) else 0.0)
            cell_base_top = base_top_pt + (row_offsets[row_index] if row_index < len(row_offsets) else 0.0)
            collect_report_items_textboxes(
                cell_contents,
                namespace,
                cell_base_left,
                cell_base_top,
...
def collect_report_items_textboxes(parent_node, namespace, base_left_pt, base_top_pt, collector):
    for child in list(parent_node):
        local_name = get_xml_local_name(child)

        if local_name == "Textbox":
            local_left = get_child_mm_points(child, "Left", namespace) or 0.0
            local_top = get_child_mm_points(child, "Top", namespace) or 0.0
            absolute_left = base_left_pt + local_left
            absolute_top = base_top_pt + local_top
            collector.append((child, absolute_left, absolute_top))
            continue

        if local_name == "Rectangle":
            rectangle_left = base_left_pt + (get_child_mm_points(child, "Left", namespace) or 0.0)
            rectangle_top = base_top_pt + (get_child_mm_points(child, "Top", namespace) or 0.0)
            report_items = child.find("rdl:ReportItems", namespace)
        ...
        if local_name == "Tablix":
            tablix_left = base_left_pt + (get_child_mm_points(child, "Left", namespace) or 0.0)
            tablix_top = base_top_pt + (get_child_mm_points(child, "Top", namespace) or 0.0)
            collect_tablix_textboxes(child, namespace, tablix_left, tablix_top, collector)
            continue

        nested_report_items = child.find("rdl:ReportItems", namespace)
        if nested_report_items is not None:
            nested_left = base_left_pt + (get_child_mm_points(child, "Left", namespace) or 0.0)
            nested_top = base_top_pt + (get_child_mm_points(child, "Top", namespace) or 0.0)
            collect_report_items_textboxes(
                nested_report_items,
                namespace,
                nested_left,
...
INVENTORY_Y_OFFSET_PT = 0.0
INVENTORY_TOTAL_Y_OFFSET_PT = -16.0
DISCOUNTS_Y_OFFSET_PT = -24.0
TOTALS_Y_OFFSET_PT = -28.0

FINIQUITO_HEADER_TEXTBOXES = [
    "MES2",
    "FechaAsigna",
    "PLAZA2",
    "CRDISTRITO2",
...
FINIQUITO_INTEGRATION_TEXTBOXES = [
    "COMISIONFIJA2",
    "COMISIONFIJA4",
    "COMISIONFIJA10",
    "IMSSYSAREMPLEADOS",
    ...
    "COMISIONFIJA5",
    "COMISIONFIJA6",
    "COMISIONPORESTRUCTURA2",
    "COMISIONVARIABLE",
    "SUBTOTALCOMISON",
    "IVASUBTOTALCOMISION",
    "RETENCIONDOSTERCIOSIVA",
    "ISPT",
    "TOTALCOMISION",
    "TOTALCOMISION4",
    "TOTALCOMISION5",
    ...
    "TOTALCOMISION8",
    "TOTALCOMISION9",
    "COMISIONPORESTRUCTURA4",
    "COMISIONPORESTRUCTURA3",
    "COMISIONVARIABLE2",
    "COMISIONVARIABLE3",
    "SUBTOTALCOMISON2",
    "IVASUBTOTALCOMISION2",
    "RETENCIONDOSTERCIOSIVA2",
    "RETENCIONISR",
    "RETENCIONISR2",
    "TOTALCOMISION2",
]

FINIQUITO_INVENTORY_TEXTBOXES = [
    "MERMAMCIACERO_ANTERIOR",
    "MERMAMCIACERO_MENSUAL",
    "MERMAMCIACERO_ACUMULAD",
    "MERMAMCIACERO_FACTURAD",
    "MERMAMCIACERO_NOFAC",
    "MERMAMCIACERO_SALDO",
    "MERMAMCIAEXENTA_ANTERIOR",
    "MERMAMCIAEXENTA_MENSUAL",
    "MERMAMCIAEXENTA_ACUMULAD",
    "MERMAMCIAEXENTA_FACTURAD",
    "MERMAMCIAEXENTA_NOFAC",
    "MERMAMCIAEXENTA_SALDO",
    "MERMAMCIACONSIG_ANTERIOR",
    "MERMAMCIACONSIG_MENSUAL",
    "MERMAMCIACONSIG_ACUMULAD",
    "MERMAMCIACONSIG_FACTURAD",
    "MERMAMCIACONSIG_NOFAC",
    "MERMAMCIACONSIG_SALDO",
    "MERMAMCIAGRAVAD_ANTERIOR",
    "MERMAMCIAGRAVAD_MENSUAL",
    "MERMAMCIAGRAVAD_ACUMULAD",
    "MERMAMCIAGRAVAD_FACTURAD",
    "MERMAMCIAGRAVAD_NOFAC",
    "MERMAMCIAGRAVAD_SALDO",
    "MERMASUBTOTAL_ANTERIOR",
    "MERMASUBTOTAL_MENSUAL",
    "MERMASUBTOTAL_ACUMULAD",
    "MERMASUBTOTAL_FACTURAD",
    "MERMASUBTOTAL_NOFAC",
    "MERMASUBTOTAL_SALDO",
    "MERMAIMPESTATAL_ANTERIOR",
    "MERMAIMPESTATAL_MENSUAL",
    "MERMAIMPESTATAL_ACUMULAD",
    "MERMAIMPESTATAL_FACTURAD",
    "MERMAIMPESTATAL_NOFAC",
    "MERMAIMPESTATAL_SALDO",
    "MERMAIEPS_ANTERIOR",
    "MERMAIEPS_MENSUAL",
    "MERMAIEPS_ACUMULADA",
    "MERMAIEPS_FACTURAD",
...
FINIQUITO_DISCOUNTS_TEXTBOXES = [
    "MERMAMCIAGRAVAD_ANTERIOR2",
    "MERMAMCIAGRAVAD_MENSUAL2",
    "MERMAMCIAGRAVAD_ACUMULAD2",
    "MERMAMCIAGRAVAD_FACTURAD2",
    "MERMAMCIAGRAVAD_SALDO2",
    "MERMASUBTOTAL_ANTERIOR2",
    "MERMASUBTOTAL_MENSUAL2",
    "MERMASUBTOTAL_ACUMULAD2",
    "MERMASUBTOTAL_FACTURAD2",
    "MERMASUBTOTAL_SALDO2",
    "MERMAIVA_ANTERIOR2",
    "MERMAIVA_MENSUAL2",
    "MERMAIVA_ACUMULAD2",
    "MERMAIVA_FACTURAD2",
    "MERMAIVA_SALDO2",
    "MERMAIMPESTATAL_ANTERIOR2",
    "MERMAIMPESTATAL_MENSUAL2",
    "MERMAIMPESTATAL_ACUMULAD2",
    "MERMAIMPESTATAL_FACTURAD2",
    "MERMAIMPESTATAL_SALDO2",
    "MERMAIEPS_ANTERIOR2",
    "MERMAIEPS_MENSUAL2",
    "MERMAIEPS_ACUMULADA2",
    "MERMAIEPS_FACTURAD2",
    "MERMAIEPS_SALDO2",
    "MERMAIVAIEPS_ANTERIOR2",
    "MERMAIVAIEPS_MENSUAL2",
    "MERMAIVAIEPS_ACUMULAD2",
    "MERMAIVAIEPS_FACTURAD2",
    "MERMAIVAIEPS_SALDO2",
    "MERMAMCIACERO_ANTERIOR3",
    "MERMAMCIACERO_MENSUAL3",
    "MERMAMCIACERO_ACUMULAD3",
    "MERMAMCIACERO_FACTURAD3",
    "MERMAMCIACERO_SALDO3",
    "MERMAMCIACERO_ANTERIOR4",
    "MERMAMCIACERO_MENSUAL4",
    "MERMAMCIACERO_ACUMULAD4",
    "MERMAMCIACERO_FACTURAD4",
    "MERMAMCIACERO_SALDO4",
    "MERMAMCIAEXENTA_ANTERIOR3",
    "MERMAMCIAEXENTA_MENSUAL3",
    "MERMAMCIAEXENTA_ACUMULAD3",
    "MERMAMCIAEXENTA_FACTURAD3",
    "MERMAMCIAEXENTA_SALDO3",
    "MERMAMCIAGRAVAD_ANTERIOR3",
    "MERMAMCIAGRAVAD_MENSUAL3",
    "MERMAMCIAGRAVAD_ACUMULAD3",
    "MERMAMCIAGRAVAD_FACTURAD3",
    "MERMAMCIAGRAVAD_SALDO3",
    "MERMAIVA_ANTERIOR3",
    "MERMAIVA_MENSUAL3",
    "MERMAIVA_ACUMULAD3",
    "MERMAIVA_FACTURAD3",
    "MERMAIVA_SALDO3",
    "MERMAIMPESTATAL_ANTERIOR3",
    "MERMAIMPESTATAL_MENSUAL3",
    "MERMAIMPESTATAL_ACUMULAD3",
    "MERMAIMPESTATAL_FACTURAD3",
    "MERMAIMPESTATAL_SALDO3",
    "MERMAIVAIEPS_ANTERIOR3",
    "MERMAIVAIEPS_MENSUAL3",
    "MERMAIVAIEPS_ACUMULAD3",
    "MERMAIVAIEPS_FACTURAD3",
    "MERMAIVAIEPS_SALDO3",
    "MERMAIEPS_ANTERIOR4",
    "MERMAIEPS_MENSUAL4",
    "MERMAIEPS_ACUMULADA4",
    "MERMAIEPS_FACTURAD4",
    "MERMAIEPS_SALDO4",
    "MERMAIVAIEPS_ANTERIOR4",
    "MERMAIVAIEPS_MENSUAL4",
    "MERMAIVAIEPS_ACUMULAD4",
    "MERMAIVAIEPS_FACTURAD4",
    "MERMAIVAIEPS_SALDO4",
    "MERMAIVAIEPS_ANTERIOR5",
    "MERMAIVAIEPS_MENSUAL5",
    "MERMAIVAIEPS_ACUMULAD5",
    "MERMAIVAIEPS_FACTURAD5",
    "MERMAIVAIEPS_SALDO5",
    "MERMAIEPS_ANTERIOR6",
    "MERMAIEPS_MENSUAL6",
    "MERMAIEPS_ACUMULADA6",
    "MERMAIEPS_FACTURAD6",
    "MERMAIEPS_SALDO6",
    "MERMAIVAIEPS_ANTERIOR6",
    "MERMAIVAIEPS_MENSUAL6",
    "MERMAIVAIEPS_ACUMULAD6",
    "MERMAIVAIEPS_FACTURAD6",
    "MERMAIVAIEPS_SALDO6",
]

FINIQUITO_TOTALS_TEXTBOXES = [
    "Textbox128",
    "Textbox129",
    "Textbox130",
    "Textbox131",
...
FINIQUITO_DISCOUNTS_TOP_TEXTBOXES = [
    "MERMAMCIAGRAVAD_ANTERIOR2",
    "MERMAMCIAGRAVAD_MENSUAL2",
    "MERMAMCIAGRAVAD_ACUMULAD2",
    "MERMAMCIAGRAVAD_FACTURAD2",
...
FINIQUITO_DISCOUNTS_UPPER_ROWS_TEXTBOXES = [
    "MERMAMCIAGRAVAD_ANTERIOR2",
    "MERMAMCIAGRAVAD_MENSUAL2",
    "MERMAMCIAGRAVAD_ACUMULAD2",
    "MERMAMCIAGRAVAD_FACTURAD2",
...
FINIQUITO_INTEGRATION_TEXTBOX_SET = {name.upper() for name in FINIQUITO_INTEGRATION_TEXTBOXES}
FINIQUITO_INVENTORY_TEXTBOX_SET = {name.upper() for name in FINIQUITO_INVENTORY_TEXTBOXES}
FINIQUITO_DISCOUNTS_TEXTBOX_SET = {name.upper() for name in FINIQUITO_DISCOUNTS_TEXTBOXES}
FINIQUITO_DISCOUNTS_TOP_TEXTBOX_SET = {name.upper() for name in FINIQUITO_DISCOUNTS_TOP_TEXTBOXES}
FINIQUITO_DISCOUNTS_UPPER_ROWS_TEXTBOX_SET = {name.upper() for name in FINIQUITO_DISCOUNTS_UPPER_ROWS_TEXTBOXES}
FINIQUITO_TOTALS_TEXTBOX_SET = {name.upper() for name in FINIQUITO_TOTALS_TEXTBOXES}
FINIQUITO_WHITE_CURRENCY_TEXTBOXES = {
    "TEXTBOX65",
    "TEXTBOX66",
    "TEXTBOX67",
    "TEXTBOX68",
    "TEXTBOX69",
    "TEXTBOX70",
    *FINIQUITO_TOTALS_TEXTBOX_SET,
}

FINIQUITO_TOP_TRANSFORMS = (
    (FINIQUITO_INTEGRATION_TEXTBOX_SET, 2.5017622505549157, -60.64836768646927),
    (FINIQUITO_INVENTORY_TEXTBOX_SET, 1.7128994915314972, 44.35745278361094),
    (FINIQUITO_DISCOUNTS_UPPER_ROWS_TEXTBOX_SET, 2.064563436491372, -68.24302919471643),
    (FINIQUITO_DISCOUNTS_TOP_TEXTBOX_SET, 2.064563436491372, -60.24302919471643),
    (FINIQUITO_DISCOUNTS_TEXTBOX_SET, 2.064563436491372, -66.24302919471643),
    (FINIQUITO_TOTALS_TEXTBOX_SET, 1.8292315434014281, 19.959436634662325),
)

FINIQUITO_HEADER_POSITIONS = {
    "MES": {"x": 122.99, "y": 68.33, "max_width": 110},
    "PLAZA": {"x": 122.99, "y": 81.20, "max_width": 140},
    "CRDISTRITO": {"x": 122.99, "y": 89.55, "max_width": 150},
    "TIENDA": {"x": 122.99, "y": 97.90, "max_width": 185},
    "NOMBRECOMISIONISTA": {"x": 122.99, "y": 106.30, "max_width": 235},
    "RFC": {"x": 340.58, "y": 54.15, "max_width": 90},
    "FECHAINICIO": {"x": 383.73, "y": 68.40, "max_width": 40, "font_size": 6.2},
    "FECHAFIN": {"x": 443.97, "y": 68.40, "max_width": 40, "font_size": 6.2},
}

FINIQUITO_INTEGRATION_TEXTBOX_OFFSETS_PT = {
    "COMISIONFIJA2": {"x": 0.0, "y": 0.0},
    "COMISIONFIJA4": {"x": 0.0, "y": 0.7},
    "COMISIONFIJA10": {"x": 0.0, "y": 0.0},
    "IMSSYSAREMPLEADOS": {"x": 0.0, "y": 0.0},
    "COMISIONFIJA7": {"x": 0.0, "y": 0.0},
    "COMISIONFIJA8": {"x": 0.0, "y": 0.0},
    "COMISIONFIJA5": {"x": 0.0, "y": 0.0},
    "COMISIONFIJA6": {"x": 0.0, "y": 0.7},
    "COMISIONPORESTRUCTURA2": {"x": 0.0, "y": 0.0},
    "COMISIONVARIABLE": {"x": 0.0, "y": 1.3},
    "SUBTOTALCOMISON": {"x": 0.0, "y": 1.2},
    "IVASUBTOTALCOMISION": {"x": 0.0, "y": 1.0},
    "RETENCIONDOSTERCIOSIVA": {"x": 0.0, "y": 1.1},
    "TOTALCOMISION": {"x": 0.0, "y": 0.8},
    "TOTALCOMISION4": {"x": 0.0, "y": 0.8},
    "TOTALCOMISION5": {"x": 0.0, "y": 1.0},
    "TOTALCOMISION6": {"x": 0.0, "y": 0.9},
    "TOTALCOMISION7": {"x": 0.0, "y": 1.2},
    "TOTALCOMISION8": {"x": 0.0, "y": 1.3},
    "TOTALCOMISION9": {"x": 0.0, "y": 0.5},
    "COMISIONPORESTRUCTURA4": {"x": 0.0, "y": 1.0},
    "COMISIONPORESTRUCTURA3": {"x": 0.0, "y": 0.5},
    "COMISIONVARIABLE2": {"x": 0.0, "y": 0.2},
    "COMISIONVARIABLE3": {"x": 0.0, "y": 0.1},
    "SUBTOTALCOMISON2": {"x": 0.0, "y": 0.0},
    "IVASUBTOTALCOMISION2": {"x": 0.0, "y": 0.0},
    "RETENCIONDOSTERCIOSIVA2": {"x": 0.0, "y": 0.0},
    "RETENCIONISR": {"x": 0.0, "y": 0.0},
    "RETENCIONISR2": {"x": 0.0, "y": -0.2},
    "TOTALCOMISION2": {"x": 0.0, "y": -0.8},
}

FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT = {
    "ANTERIOR": 1.2,
    "MENSUAL": 1.0,
    "ACUMULADA": 1.0,
    "FACTURADA": 0.0,
...
SALDO_Y = mm_to_points("0.11mm")

FINIQUITO_INVENTORY_TEXTBOX_Y_OFFSETS_PT = {
    "MERMAMCIACERO_MENSUAL": mm_to_points("0.13892mm"),
    "MERMAMCIACERO_ACUMULAD": mm_to_points("0.13892mm"),
    "MERMAMCIAEXENTA_MENSUAL": mm_to_points("0.12862mm"),
    "MERMAMCIAEXENTA_ACUMULAD": mm_to_points("0.12862mm"),
    "MERMAMCIACONSIG_MENSUAL": mm_to_points("0.12418mm"),
    "MERMAMCIACONSIG_ACUMULAD": mm_to_points("0.12418mm"),
    "MERMAIMPESTATAL_MENSUAL": mm_to_points("0.25279mm"),
    "MERMAIMPESTATAL_ACUMULAD": mm_to_points("0.25279mm"),
    "MERMAIEPS_MENSUAL": mm_to_points("-0.03167mm"),
    "MERMAIVA_MENSUAL": mm_to_points("0.02114mm"),
    "MERMAIVA_ACUMULAD": mm_to_points("0.02114mm"),
    "MERMAMCIACERO_NOFAC": mm_to_points("0.13892mm"),
    "MERMAMCIACERO_SALDO": SALDO_Y,
    "MERMAMCIAEXENTA_SALDO": SALDO_Y,
    "MERMAMCIACONSIG_SALDO": SALDO_Y,
    "MERMAIMPESTATAL_NOFAC": mm_to_points("0.09999mm"),
    "MERMAIVAIEPS_NOFAC": mm_to_points("0.33164mm"),
    "MERMAIEPS_SALDO": SALDO_Y,
    "MERMAIVA_SALDO": SALDO_Y,
}

FINIQUITO_INVENTORY_TOTAL_ROW_X_OFFSETS_PT = {
    "TEXTBOX65": -0.5,  # Anterior
    "TEXTBOX66": -0.5,  # Mensual
    "TEXTBOX70": -0.5,  # Acumulada
    "TEXTBOX67": -0.5,  # Facturada
    "TEXTBOX68": -0.5,  # NoFacturable
    "TEXTBOX69": -0.5,  # Saldo
}

FINIQUITO_TOTALS_TEXTBOX_OFFSETS_PT = {
    "TOTAL_ANTICIPOS_LABEL": {"x": 0.0, "y": 0.0},
    "TOTAL_ANTICIPOS_VALUES": {"x": 1.0, "y": -0.2},
    "TEXTBOX1": {"x": 0.0, "y": 0.0},  # Total a Pagar
    "TEXTBOX2": {"x": 0.0, "y": -2.5},  # Flujo Mensual
}

FINIQUITO_DISCOUNTS_COLUMN_OFFSETS_PT = {
    "ANTERIOR": -0.3,
    "MENSUAL": -0.5,
    "ACUMULADA": -0.7,
    "RECUPERACION": -1.4,
...
def build_textbox_y_overrides(groups, row_offsets=None):
    overrides = {}
    row_offsets = row_offsets or {}
    for row_key, top_y, textbox_names in groups:
        adjusted_top_y = top_y - row_offsets.get(row_key, 0.0)
        for textbox_name in textbox_names:
            overrides[textbox_name.upper()] = adjusted_top_y
    return overrides


FINIQUITO_DISCOUNTS_ROW_GROUPS = [
    ("RETENCION_IVA", 499.76, ["MERMAMCIAGRAVAD_ANTERIOR2", "MERMAMCIAGRAVAD_MENSUAL2", "MERMAMCIAGRAVAD_ACUMULAD2", "MERMAMCIAGRAVAD_FACTURAD2", "MERMAMCIAGRAVAD_SALDO2"]),
    ("FIDEICOMISO_426", 508.91, ["MERMASUBTOTAL_ANTERIOR2", "MERMASUBTOTAL_MENSUAL2", "MERMASUBTOTAL_ACUMULAD2", "MERMASUBTOTAL_FACTURAD2", "MERMASUBTOTAL_SALDO2"]),
    ("FIDEICOMISO_426_DESCUENTO", 520.07, ["MERMAIVA_ANTERIOR2", "MERMAIVA_MENSUAL2", "MERMAIVA_ACUMULAD2", "MERMAIVA_FACTURAD2", "MERMAIVA_SALDO2"]),
    ("ADELANTO_COMISION_FIJA", 532.88, ["MERMAIMPESTATAL_ANTERIOR2", "MERMAIMPESTATAL_MENSUAL2", "MERMAIMPESTATAL_ACUMULAD2", "MERMAIMPESTATAL_FACTURAD2", "MERMAIMPESTATAL_SALDO2"]),
    ("RESERVA_AGUINALDO", 542.17, ["MERMAIEPS_ANTERIOR2", "MERMAIEPS_MENSUAL2", "MERMAIEPS_ACUMULADA2", "MERMAIEPS_FACTURAD2", "MERMAIEPS_SALDO2"]),
    ("ANTICIPO_PAGO_SALARIOS", 556.00, ["MERMAIVAIEPS_ANTERIOR2", "MERMAIVAIEPS_MENSUAL2", "MERMAIVAIEPS_ACUMULAD2", "MERMAIVAIEPS_FACTURAD2", "MERMAIVAIEPS_SALDO2"]),
    ("ANTICIPOS_ESPECIALES", 570.61, ["MERMAMCIACERO_ANTERIOR3", "MERMAMCIACERO_MENSUAL3", "MERMAMCIACERO_ACUMULAD3", "MERMAMCIACERO_FACTURAD3", "MERMAMCIACERO_SALDO3"]),
    ("ANTICIPO_DESPACHO_CONTABLE", 583.28, ["MERMAMCIACERO_ANTERIOR4", "MERMAMCIACERO_MENSUAL4", "MERMAMCIACERO_ACUMULAD4", "MERMAMCIACERO_FACTURAD4", "MERMAMCIACERO_SALDO4"]),
    ("DEVOLUCION_RETENCION_DIFERENCIA_IVA", 597.75, ["MERMAMCIAEXENTA_ANTERIOR3", "MERMAMCIAEXENTA_MENSUAL3", "MERMAMCIAEXENTA_ACUMULAD3", "MERMAMCIAEXENTA_FACTURAD3", "MERMAMCIAEXENTA_SALDO3"]),
    ("DEVOLUCION_RESERVA_AGUINALDO", 611.58, ["MERMAMCIAGRAVAD_ANTERIOR3", "MERMAMCIAGRAVAD_MENSUAL3", "MERMAMCIAGRAVAD_ACUMULAD3", "MERMAMCIAGRAVAD_FACTURAD3", "MERMAMCIAGRAVAD_SALDO3"]),
    ("RETENCION_IMPUESTOS", 627.13, ["MERMAIMPESTATAL_ANTERIOR3", "MERMAIMPESTATAL_MENSUAL3", "MERMAIMPESTATAL_ACUMULAD3", "MERMAIMPESTATAL_FACTURAD3", "MERMAIMPESTATAL_SALDO3"]),
    ("RETENCION_IMPUESTOS_NOMINA", 645.12, ["MERMAIVA_ANTERIOR3", "MERMAIVA_MENSUAL3", "MERMAIVA_ACUMULAD3", "MERMAIVA_FACTURAD3", "MERMAIVA_SALDO3"]),
    ("ANTICIPO_PAGO_FACTURAS", 661.04, ["MERMAIVAIEPS_ANTERIOR3", "MERMAIVAIEPS_MENSUAL3", "MERMAIVAIEPS_ACUMULAD3", "MERMAIVAIEPS_FACTURAD3", "MERMAIVAIEPS_SALDO3"]),
    ("SEGURO_VOLUNTARIO", 672.64, ["MERMAIEPS_ANTERIOR4", "MERMAIEPS_MENSUAL4", "MERMAIEPS_ACUMULADA4", "MERMAIEPS_FACTURAD4", "MERMAIEPS_SALDO4"]),
    ("ANTICIPO_PAGO_MULTAS", 683.87, ["MERMAIVAIEPS_ANTERIOR4", "MERMAIVAIEPS_MENSUAL4", "MERMAIVAIEPS_ACUMULAD4", "MERMAIVAIEPS_FACTURAD4", "MERMAIVAIEPS_SALDO4"]),
    ("DESCUENTO_FAST_FOOD", 695.60, ["MERMAIVAIEPS_ANTERIOR5", "MERMAIVAIEPS_MENSUAL5", "MERMAIVAIEPS_ACUMULAD5", "MERMAIVAIEPS_FACTURAD5", "MERMAIVAIEPS_SALDO5"]),
    ("DIFERENCIA_DEPOSITOS", 708.84, ["MERMAIEPS_ANTERIOR6", "MERMAIEPS_MENSUAL6", "MERMAIEPS_ACUMULADA6", "MERMAIEPS_FACTURAD6", "MERMAIEPS_SALDO6"]),
    ("OTROS_ANTICIPOS", 717.78, ["MERMAIVAIEPS_ANTERIOR6", "MERMAIVAIEPS_MENSUAL6", "MERMAIVAIEPS_ACUMULAD6", "MERMAIVAIEPS_FACTURAD6", "MERMAIVAIEPS_SALDO6"]),
]

# Positivo sube la fila; negativo la baja.
FINIQUITO_DISCOUNTS_ROW_OFFSETS_PT = {
    "RETENCION_IVA": 0.7,
    "FIDEICOMISO_426": -0.2,
    "FIDEICOMISO_426_DESCUENTO": -0.1,
    "ADELANTO_COMISION_FIJA": 0.7,
    "RESERVA_AGUINALDO": 0.0,
    "ANTICIPO_PAGO_SALARIOS": 2.0,
    "ANTICIPOS_ESPECIALES": -0.2,
    "ANTICIPO_DESPACHO_CONTABLE": -0.1,
    "DEVOLUCION_RETENCION_DIFERENCIA_IVA": 0.5,
    "DEVOLUCION_RESERVA_AGUINALDO": 3.0,
    "RETENCION_IMPUESTOS": 0.6,
    "RETENCION_IMPUESTOS_NOMINA": 0.6,
    "ANTICIPO_PAGO_FACTURAS": -0.1,
    "SEGURO_VOLUNTARIO": -0.2,
    "ANTICIPO_PAGO_MULTAS": -0.1,
    "DESCUENTO_FAST_FOOD": -0.2,
    "DIFERENCIA_DEPOSITOS": 0.5,
    "OTROS_ANTICIPOS": -2.5,
}

FINIQUITO_DISCOUNTS_ROW_TOP_OVERRIDES = build_textbox_y_overrides(
    FINIQUITO_DISCOUNTS_ROW_GROUPS,
    row_offsets=FINIQUITO_DISCOUNTS_ROW_OFFSETS_PT
)
...
def parse_rdl_metadata(rdl_path):
    ...
    left_margin_node = root.find("./rdl:ReportSections/rdl:ReportSection/rdl:Page/rdl:LeftMargin", namespace)
    top_margin_node = root.find("./rdl:ReportSections/rdl:ReportSection/rdl:Page/rdl:TopMargin", namespace)
    page_info = {
        "left_margin_pt": mm_to_points(left_margin_node.text) if left_margin_node is not None and left_margin_node.text else 0.0,
        "top_margin_pt": mm_to_points(top_margin_node.text) if top_margin_node is not None and top_margin_node.text else 0.0,
    }

    textbox_nodes = []
    body_report_items = root.find(
        "./rdl:ReportSections/rdl:ReportSection/rdl:Body/rdl:ReportItems",
        namespace,
    )
    if body_report_items is not None:
        collect_report_items_textboxes(body_report_items, namespace, 0.0, 0.0, textbox_nodes)
    else:
        for textbox in root.findall(".//rdl:Textbox", namespace):
            textbox_nodes.append((textbox, 0.0, 0.0))

    for textbox, absolute_left_pt, absolute_top_pt in textbox_nodes:
        ...
        textbox_info = {
            ...
            "left_pt": absolute_left_pt if absolute_left_pt is not None else (mm_to_points(left_node.text) if left_node is not None and left_node.text else None),
            "top_pt": absolute_top_pt if absolute_top_pt is not None else (mm_to_points(top_node.text) if top_node is not None and top_node.text else None),
...
def get_finiquito_transformed_top_y(textbox_name, top_pt):
    upper_name = str(textbox_name or "").upper()
    if top_pt is None or not upper_name:
        return None

    if upper_name in FINIQUITO_DISCOUNTS_ROW_TOP_OVERRIDES:
        return FINIQUITO_DISCOUNTS_ROW_TOP_OVERRIDES[upper_name]

    for textbox_set, slope, intercept in FINIQUITO_TOP_TRANSFORMS:
        if upper_name in textbox_set:
            return (slope * top_pt) + intercept

    return None


def get_finiquito_integration_textbox_offset(textbox_name, axis):
    textbox_offsets = FINIQUITO_INTEGRATION_TEXTBOX_OFFSETS_PT.get(str(textbox_name or "").upper(), {})
    try:
        return float(textbox_offsets.get(axis, 0.0))
    except (TypeError, ValueError):
        return 0.0


def get_finiquito_totals_textbox_offset(textbox_name, axis):
    upper_name = str(textbox_name or "").upper()
    if upper_name == "TEXTBOX128":
        offset_key = "TOTAL_ANTICIPOS_LABEL"
    elif upper_name in {"TEXTBOX129", "TEXTBOX130", "TEXTBOX131", "TOTALRECUP", "TEXTBOX133"}:
        offset_key = "TOTAL_ANTICIPOS_VALUES"
    else:
        offset_key = upper_name
    textbox_offsets = FINIQUITO_TOTALS_TEXTBOX_OFFSETS_PT.get(offset_key, {})
    try:
        return float(textbox_offsets.get(axis, 0.0))
    except (TypeError, ValueError):
        return 0.0
...
def get_finiquito_inventory_column_offset(textbox_name):
    upper_name = str(textbox_name or "").upper()

    total_row_offset = FINIQUITO_INVENTORY_TOTAL_ROW_X_OFFSETS_PT.get(upper_name)
    if total_row_offset is not None:
        return total_row_offset

    if upper_name == "TEXTBOX65":
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("ANTERIOR", 0.0)
    if upper_name == "TEXTBOX66":
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("MENSUAL", 0.0)
    if upper_name == "TEXTBOX70":
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("ACUMULADA", 0.0)
    if upper_name == "TEXTBOX67":
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("FACTURADA", 0.0)
    if upper_name == "TEXTBOX68":
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("NOFACTURABLE", 0.0)
    if upper_name == "TEXTBOX69":
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("SALDO", 0.0)

    if "_ANTERIOR" in upper_name:
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("ANTERIOR", 0.0)
    if "_MENSUAL" in upper_name:
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("MENSUAL", 0.0)
    if "_ACUMULAD" in upper_name:
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("ACUMULADA", 0.0)
    if "_FACTURAD" in upper_name:
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("FACTURADA", 0.0)
    if "_NOFAC" in upper_name:
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("NOFACTURABLE", 0.0)
    if "_SALDO" in upper_name:
        return FINIQUITO_INVENTORY_COLUMN_OFFSETS_PT.get("SALDO", 0.0)

    return 0.0


def get_finiquito_inventory_textbox_y_offset(textbox_name):
    try:
        return float(FINIQUITO_INVENTORY_TEXTBOX_Y_OFFSETS_PT.get(str(textbox_name or "").upper(), 0.0))
    except (TypeError, ValueError):
        return 0.0


def get_finiquito_discounts_column_offset(textbox_name):
    upper_name = str(textbox_name or "").upper()

    if upper_name == "TEXTBOX129":
        return FINIQUITO_DISCOUNTS_COLUMN_OFFSETS_PT.get("ANTERIOR", 0.0)
    if upper_name == "TEXTBOX130":
        return FINIQUITO_DISCOUNTS_COLUMN_OFFSETS_PT.get("MENSUAL", 0.0)
    if upper_name == "TEXTBOX131":
        return FINIQUITO_DISCOUNTS_COLUMN_OFFSETS_PT.get("ACUMULADA", 0.0)
    if upper_name == "TOTALRECUP":
        return FINIQUITO_DISCOUNTS_COLUMN_OFFSETS_PT.get("RECUPERACION", 0.0)
    if upper_name == "TEXTBOX133":
        return FINIQUITO_DISCOUNTS_COLUMN_OFFSETS_PT.get("SALDO", 0.0)
...
def get_currency_text_rgb(textbox_name):
    upper_name = str(textbox_name or "").upper()
    if upper_name in {"TEXTBOX1", "TEXTBOX2"}:
        return (0, 0, 0)
    if upper_name in FINIQUITO_WHITE_CURRENCY_TEXTBOXES:
        return (1, 1, 1)
    return None


def get_rdl_canvas_position(page_height, textbox_info, align="left", textbox_name=None):
    if not textbox_info:
        return None
    ...
    height_pt = textbox_info.get("height_pt")
    if left_pt is None or top_pt is None:
        return None

    page_info = textbox_info.get("page") or {}
    left_margin_pt = page_info.get("left_margin_pt", 0.0) or 0.0
    top_margin_pt = page_info.get("top_margin_pt", 0.0) or 0.0
    font_size_pt = parse_reportlab_font_size(textbox_info.get("font_size"), 8)
    baseline_offset_pt = max(height_pt or 0.0, font_size_pt * 0.78)
    horizontal_padding_pt = 1.5

    x = left_margin_pt + left_pt + (0 if align == "right" else horizontal_padding_pt)
    if align == "right" and width_pt is not None:
        x += width_pt - horizontal_padding_pt
    x += get_finiquito_integration_textbox_offset(textbox_name, "x")
    x += get_finiquito_inventory_column_offset(textbox_name)
    x += get_finiquito_totals_textbox_offset(textbox_name, "x")
    x += get_finiquito_discounts_column_offset(textbox_name)

    transformed_top_y = get_finiquito_transformed_top_y(textbox_name, top_pt)
    if transformed_top_y is not None:
        y = page_height - transformed_top_y - baseline_offset_pt
    else:
        y = page_height - top_margin_pt - top_pt - baseline_offset_pt
    y += get_finiquito_integration_textbox_offset(textbox_name, "y")
    y += get_finiquito_inventory_textbox_y_offset(textbox_name)
    y += get_finiquito_totals_textbox_offset(textbox_name, "y")
    return {
        "x": x,
        "y": y,
        "width": width_pt,
...
def get_rdl_field_canvas_position(layout, page_height, field_name, align="left"):
    return get_rdl_canvas_position(
        page_height,
        get_rdl_field_expression(layout, field_name),
        align=align,
        textbox_name=field_name,
    )
...
def get_rdl_field_canvas_position_in_range(layout, page_height, field_name, min_top=None, max_top=None, align="left"):
    ...
    return get_rdl_canvas_position(page_height, filtered[0], align=align, textbox_name=field_name)
...
NUMERIC_FIELD_SOURCE_OVERRIDES = {
    # Este campo en el PDF detallado debe salir de la columna
    # COMISIONEXTRAORDINARIAPC del CSV, aunque el renglón conserve
    # el nombre historico comisionextraordinariaca.
    "comisionextraordinariaca": "comisionextraordinariapc",
}


def calculate_ispt_value(row):
    """
    ISPT no siempre llega como columna directa; en el PDF se construye con la
    formula de la comision por estructura menos los conceptos de costo laboral.
    """
    return (
        safe_float(get_row_value(row, "COMISIONPORESTRUCTURA"), 0.0)
        - (
            safe_float(get_row_value(row, "SUELDOEMPLEADOS"), 0.0)
            + safe_float(get_row_value(row, "IMSSYSAREMPLEADOS"), 0.0)
            + safe_float(get_row_value(row, "INFONAVITEMPLEADOS"), 0.0)
            + safe_float(get_row_value(row, "ISNEMPLEADOS"), 0.0)
            + safe_float(get_row_value(row, "PRIMAVACACIONALEMPLEADOS"), 0.0)
            + safe_float(get_row_value(row, "AGUINALDOEMPLEADOSPAGO"), 0.0)
        )
    )


def calculate_aguinaldo_total_value(row):
    """
    En la plantilla detallada este renglón representa la suma de prima vacacional
    y aguinaldo pagado, no solo el campo AGUINALDOEMPLEADOSPAGO.
    """
    return (
        safe_float(get_row_value(row, "PRIMAVACACIONALEMPLEADOS"), 0.0)
        + safe_float(get_row_value(row, "AGUINALDOEMPLEADOSPAGO"), 0.0)
...
def get_numeric_field_or_rdl(layout, row, field_name):
    source_field_name = NUMERIC_FIELD_SOURCE_OVERRIDES.get(str(field_name or "").lower(), field_name)

    if str(source_field_name or "").lower() == "ispt":
        return calculate_ispt_value(row)
    if str(source_field_name or "").lower() == "aguinaldoempleadospago":
        return calculate_aguinaldo_total_value(row)

    if source_field_name != field_name:
        override_value = get_row_value(row, source_field_name)
        if override_value not in (None, ""):
            return safe_float(override_value, 0.0)
...
def resolve_rdl_textbox_render_value(layout, textbox_name, textbox_info, row):
    ...
    if direct_field_match:
        ...
        if field_name.upper() in {"ISPT", "AGUINALDOEMPLEADOSPAGO"}:
            return get_numeric_field_or_rdl(layout, row, field_name), "currency"
        if raw_value in (None, ""):
            return get_numeric_field_or_rdl(layout, row, field_name), "currency"
        return raw_value, "currency"

    upper_expression = expression.upper()
    if "FORMAT(FIELDS!FECHAINICIO.VALUE" in upper_expression and "FORMAT(FIELDS!FECHAFIN.VALUE" in upper_expression:
        fecha_inicio = format_date_value(get_row_value(row, "FechaInicio"))
        fecha_fin = format_date_value(get_row_value(row, "FechaFin"))
        return f"Del:   {fecha_inicio}   al   {fecha_fin}", "text"
...
def draw_rdl_textboxes_in_range(pdf_canvas, page_height, row, layout, min_top_mm, max_top_mm):
    ...
    for textbox_name, textbox_info in ordered_items:
        ...
        render_value, render_mode = resolve_rdl_textbox_render_value(layout, textbox_name, textbox_info, row)
        ...
        if render_mode == "currency":
            position = get_rdl_canvas_position(page_height, textbox_info, align="right", textbox_name=textbox_name)
            if not position:
                continue
            draw_right_currency(
                pdf_canvas,
                ...
                font_size=font_size,
                max_width=max_width,
                text_rgb=get_currency_text_rgb(textbox_name)
            )
            continue

        position = get_rdl_canvas_position(page_height, textbox_info, align="left", textbox_name=textbox_name)
        if not position:
            continue
        if textbox_name.upper() == "TIENDA2":
            draw_text(
                pdf_canvas,
                position["x"],
                position["y"],
...
def draw_finiquito_header(pdf_canvas, page_height, row):
    header_font_size = 6.2

    def draw_header_field(field_name, raw_value, max_width=None):
        position = FINIQUITO_HEADER_POSITIONS[field_name]
        draw_text(
            pdf_canvas,
            position["x"],
            page_height_from_top(page_height, position["y"]),
            raw_value,
            font_name="Helvetica",
            font_size=position.get("font_size", header_font_size),
            max_width=max_width or position.get("max_width"),
            min_size=6,
            trim_overflow=True,
        )

    draw_header_field("MES", get_row_value(row, "MES"))
    draw_header_field("PLAZA", get_row_value(row, "PLAZA"))
    draw_header_field("CRDISTRITO", get_row_value(row, "CRDISTRITO"))
    draw_header_field("TIENDA", get_row_value(row, "TIENDA"))
    draw_header_field("NOMBRECOMISIONISTA", get_row_value(row, "NOMBRECOMISIONISTA"))
    draw_header_field(
        "FECHAINICIO",
        format_date_value(get_first_row_value(row, ["FechaInicio", "FECHAINICIO", "FECHAINICIAL"]))
    )
    draw_header_field(
        "FECHAFIN",
        format_date_value(get_first_row_value(row, ["FechaFin", "FECHAFIN", "FECHAFINAL"]))
    )


def page_height_from_top(page_height, top_y):
    return page_height - top_y - 6.2
...
def load_template_layout(script_config):
    ...
    rdl_path = resolve_path(
        get_config_value(config_section, "LOCAL_TEMPLATE_RDL_PATH", find_default_rdl_path()),
        runtimeProjectRoot
    )
    if rdl_path and not os.path.exists(rdl_path):
        rdl_path = resolve_path(
            get_config_value(config_section, "LOCAL_TEMPLATE_RDL_PATH", find_default_rdl_path()),
            projectRoot
        )
    ...
    layout_path = resolve_path(
        get_config_value(config_section, "LOCAL_TEMPLATE_LAYOUT_PATH", find_default_layout_path()),
        runtimeProjectRoot
    )
    ...
    layout["resolved_rdl_path"] = rdl_path
...
def draw_finiquito_overlay(page_width, page_height, row, layout):
    ...
    # Limpiamos esas dos zonas antes de escribir las fechas finales.
    pdf_canvas.saveState()
    pdf_canvas.setFillColorRGB(1, 1, 1)
    pdf_canvas.rect(397.0, 760.5, 44.0, 14.0, stroke=0, fill=1)
    pdf_canvas.rect(451.0, 760.5, 44.0, 14.0, stroke=0, fill=1)
    ...
    for source_key, fallback_y_pt, value, is_total in integration_rows:
        row_cfg = integration_rows_layout.get(source_key, {}) if isinstance(integration_rows_layout, dict) else {}
        row_y = safe_float(row_cfg.get("y_pt"), fallback_y_pt) if row_cfg else fallback_y_pt
        font_size = integration_total_font_size if is_total else integration_default_font_size
        max_width = integration_total_max_width if is_total else integration_default_max_width
        draw_right_currency(
            pdf_canvas,
            integration_right_x,
            row_y,
    ...
    inventory_row_y_offset_pt = -3.4
    inventory_row_extra_offsets_pt = {
        "MERMAMCIACONSIG": 1.5,
        "MERMAMCIAGRAVAD": 2.0,
        "MERMASUBTOTAL": 3.1,
        "MERMAIVA": 3.6,
    ...
    for prefix in inventory_row_prefixes:
        ...
        row_y += inventory_row_y_offset_pt
        row_y += inventory_row_extra_offsets_pt.get(prefix, 0.0)
        source_prefix = inventory_source_prefix_override.get(prefix, prefix)
        for suffix in inventory_suffixes:
            field_name = f"{source_prefix}_{inventory_field_suffix_lookup[suffix]}"
    ...
    for suffix in inventory_suffixes:
        ...
        draw_right_currency(
            ...
            max_width=inventory_max_width,
            text_rgb=(1, 1, 1) if inventory_total_white else None,
        )

    discounts_columns = discounts_layout.get("columns") if discounts_layout else {}
    discounts_column_right_x = {
        "ANTERIOR": safe_float(((discounts_columns.get("ANTERIOR") or {}).get("right_x_pt") if isinstance(discounts_columns, dict) else None), 314.95),
        "MENSUAL": safe_float(((discounts_columns.get("MENSUAL") or {}).get("right_x_pt") if isinstance(discounts_columns, dict) else None), 375.47),
        "ACUM": safe_float(((discounts_columns.get("ACUM") or {}).get("right_x_pt") if isinstance(discounts_columns, dict) else None), 435.99),
        "RECUP": safe_float(((discounts_columns.get("RECUP") or {}).get("right_x_pt") if isinstance(discounts_columns, dict) else None), 496.51),
        "SALDO": safe_float(((discounts_columns.get("SALDO") or {}).get("right_x_pt") if isinstance(discounts_columns, dict) else None), 557.03),
    }
    discounts_rows = [
        ("RETENCION_IVA", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "RETENUNTERCIOIVA_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "RETENUNTERCIOIVA_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "RETENCIONUNTERCIO_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "RETENUNTERCIOIVA_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "RETENUNTERCIOIVA_SALDO"),
        }),
        ("FIDEICOMISO_426", {
            "ANTERIOR": 0.0,
            "MENSUAL": 0.0,
        ...
        ("RESERVA_AGUINALDO", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "RESERAGUINALRED_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "RESERAGUINREDPAT_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "RESERAGUINRED_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "RESERAGUINRED_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "RESERAGUINRED_SALDO"),
        }),
        ("ANTICIPO_PAGO_SALARIOS", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "ANTICIPOPAGOSAL_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "ANTIPAGOSALARIOS_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "ANTIPAGOSALARIOS_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "ANTIPAGOSALARIOS_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "ANTIPAGOSALARIOS_SALDO"),
        }),
        ("ANTICIPOS_ESPECIALES", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "ANTICIPOSPECIAL_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "ANTIESPECIAL_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "ANTIESPECIALES_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "ANTIESPECIALES_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "ANTIESPECIALES_SALDO"),
        }),
        ("ANTICIPO_DESPACHO_CONTABLE", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "ANTICIPOCONT_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "ANTIDESPCONT_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "ANTIDESPCONT_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "ANTIDESPCONT_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "ANTIDESPCONTABLE_SALDO"),
        }),
        ("DEVOLUCION_RETENCION_DIFERENCIA_IVA", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "DEVRETDIFIVA_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "DEVRETDIFIVA_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "DEVRETENDIFIVA_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "DEVRETENDIFIVA_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "DEVRETENDIFIVA_SALDO"),
        }),
        ("DEVOLUCION_RESERVA_AGUINALDO", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "DEV_AGUINALDO_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "DEV_AGUINALDO_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "DEV_AGUINALDO_ACUMULADA"),
        ...
        ("ANTICIPO_PAGO_FACTURAS", {
            ...
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "ANTIIMPRFACT_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "ANTIIMPRFACT_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "ANTIIMPRFACT_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "ANTIIMPFACT_SALDO"),
        }),
        ("SEGURO_VOLUNTARIO", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "DESCUENTOSEGVOL_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "DESCSEGVOLUN_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "DESCSEGVOLUN_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "DESCSEGVOLUN_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "DESCSEGDIFIVA_SALDO"),
        }),
        ("ANTICIPO_PAGO_MULTAS", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "ANTICIPOPAMULTA_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "ANTIPAGOMULTAS_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "ANTIPAGOMULTAS_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "ANTIPAGOMULTAS_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "ANTIPAGOMULTAS_SALDO"),
        }),
        ("PAGO_TOTAL_ANTICIPOS_ESPECIALES", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "anticipesptotal_anterior"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "pagototantiespe_mensual"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "pagototantiespe_acum"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "pagototantiespe_recup"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "pagototantiespecia_saldo"),
        }),
        ("DESCUENTO_FAST_FOOD", {
            "ANTERIOR": get_numeric_field_or_rdl(layout, row, "DESCUENTOFASTFO_ANTERIOR"),
            "MENSUAL": get_numeric_field_or_rdl(layout, row, "DESCFASTFOOD_MENSUAL"),
            "ACUM": get_numeric_field_or_rdl(layout, row, "DESCFASTFOOD_ACUM"),
            "RECUP": get_numeric_field_or_rdl(layout, row, "DESCUENTOFASTFOOD_RECUP"),
            "SALDO": get_numeric_field_or_rdl(layout, row, "DESCFASTFOOD_SALDO"),
        }),
    ...
    discounts_font_size = parse_float(discounts_layout.get("font_size"), 5.7) if discounts_layout else 5.7
    ...
    discounts_row_y_offset_pt = 1.2
    discounts_row_extra_offsets_pt = {
        "RETENCION_IMPUESTOS": 0.9,
    ...
    for row_index, (row_key, values_by_suffix) in enumerate(discounts_rows):
        ...
        row_y += discounts_row_y_offset_pt
        row_y += discounts_row_extra_offsets_pt.get(row_key, 0.0)
    ...
    total_anticipos_values = {
        "ANTERIOR": get_numeric_field_or_rdl(layout, row, "totalanticipos_anterior"),
        "MENSUAL": get_numeric_field_or_rdl(layout, row, "totalanticipos_mensual"),
        "ACUM": get_numeric_field_or_rdl(layout, row, "totalanticipos_acum"),
        "RECUP": get_numeric_field_or_rdl(layout, row, "totalanticipos_recup"),
        "SALDO": get_numeric_field_or_rdl(layout, row, "totalanticipos_saldo"),
    }
    for suffix, value in total_anticipos_values.items():
        draw_right_currency(
            pdf_canvas,
            discounts_column_right_x[suffix],
            discounts_total_y,
            value,
            font_name="Helvetica-Bold",
            font_size=discounts_total_font_size,
            max_width=discounts_max_width,
            text_rgb=(1, 1, 1) if discounts_total_white else None,
        )

    totals_right_x = safe_float(totals_layout.get("right_x_pt"), 557.03) if totals_layout else 557.03
    totals_max_width = safe_float(totals_layout.get("max_width_pt"), mm_to_points("24mm")) if totals_layout else mm_to_points("24mm")
    totals_font_size = parse_float(totals_layout.get("font_size"), 6.8) if totals_layout else 6.8
...
# Usamos un margen de seguridad conservador para ICM.
MAX_OUTPUT_PDF_SIZE_BYTES = int(6.3 * 1024 * 1024)
...
def generate_pdfs_from_csv_template(scriptConfig, arg1, preserve_generated_files=False):
    ...
    output_dir = ensure_directory(PDFS_FINIQUITOS_DETALLADOS_DIR)
    template_path = resolve_local_template_path(config_section)
    layout = load_template_layout(scriptConfig)
    rdl_metadata = layout.get("rdl") or {}
    resolved_rdl_path = layout.get("resolved_rdl_path")

    if not template_path or not os.path.exists(template_path):
        raise batchPDF(
            "Falta el template PDF. Coloca una plantilla en root/scripts/TemplatePDF "
            "o configura LOCAL_TEMPLATE_PATH."
        )

    if not csv_path or not os.path.exists(csv_path):
        raise batchPDF(
            "No existe el CSV de entrada configurado para generacion local: "
            f"{csv_path}. Ejecuta primero el proceso productor o configura "
            "LOCAL_TEMPLATE_CSV_PATH con la ruta compartida del CSV."
        )

    if rdl_metadata.get("path"):
        print(f"[LOCAL_TEMPLATE] RDL detectado: {rdl_metadata['path']}")
    else:
        expected_rdl_path = resolved_rdl_path or os.path.join(templateDir, "Finiquito_Detallado.rdl")
        print(
            "[LOCAL_TEMPLATE] RDL no disponible. Se usara el layout manual del template detallado: "
            f"{expected_rdl_path}"
        )
...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.info(get_frame_line(" Deleting log files older than {} ...".format(fileRetentionDays)))
